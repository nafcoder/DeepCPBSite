...
# Read training data
others = ['ESM-2-test', 'word_embedding-test']
...
y_predict, y_proba = EnsembleModel(
    torch.load('undersample_without_struct.pth'),
    torch.load('loss_without_struct.pth'),
    torch.load('weighted_without_struct.pth')
).predict(X_test)
...
writer = csv.writer(open('metrics_without_struct.csv', 'w'))
...
f = open("./outputs_without_struct/fpr.csv", "w", newline="")
writer = csv.writer(f)
writer.writerows([[fp] for fp in fpr])
f.close()

f = open("./outputs_without_struct/tpr.csv", "w", newline="")
writer = csv.writer(f)
...
f_name = f'./outputs_without_struct/precision.csv'
with open(f_name, 'w', newline='') as file:
    writer = csv.writer(file)
    writer.writerows([[p] for p in precision])

f_name = f'./outputs_without_struct/recall.csv'
with open(f_name, 'w', newline='') as file:
    writer = csv.writer(file)
